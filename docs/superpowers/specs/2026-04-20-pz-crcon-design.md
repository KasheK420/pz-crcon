...
| Frontend framework | **Next.js 15.x (App Router) + React 19.x** | Next 15 ships with React 19 by default; matches PersonalBanking + majorluk-web upgrade path |
| Language | TypeScript 5.x (strict mode) | Type safety end-to-end |
| Styling | **TailwindCSS 4.x + shadcn/ui (themed)** | Tailwind 4 + Next 15 + React 19 is the current stable combo |
...
**Auth notation:** Roles form a hierarchy `VIEWER (1) < MODERATOR (2) < ADMIN (3) < OWNER (4)`. The `+` suffix means "this role or higher". So `mod+` = MODERATOR, ADMIN, or OWNER.
...
| GET | `/api/players` | viewer+ | List with filters |
...
| GET | `/api/config/sandbox` | viewer+ | Current sandbox vars |
| PATCH | `/api/config/sandbox` | admin+ | Stage changes (apply on next restart) |
| GET | `/api/mods` | viewer+ | List installed mods |
| POST | `/api/mods` | admin+ | Add by Workshop URL/ID |
| GET | `/api/logs` | viewer+ | Filterable log query |
...
- `X-PZCrcon-Nonce: <16-byte hex>` (random per request, generated by mod)
- `X-PZCrcon-Signature: hex(HMAC_SHA256(secret, timestamp + "." + nonce + "." + body))`

Server rejects requests:
- Older than 60 seconds (timestamp window)
- With invalid HMAC
- With a (timestamp, nonce) pair already seen in the last 5 minutes (in-memory LRU; replay protection)
...
DB bootstrap (run once on first deploy against shared-postgres):

```sql
-- scripts/bootstrap-db.sql, executed manually as superuser
CREATE USER pzcrcon_user WITH PASSWORD :'PZ_DB_PASSWORD';
CREATE DATABASE pzcrcon OWNER pzcrcon_user;
GRANT ALL PRIVILEGES ON DATABASE pzcrcon TO pzcrcon_user;
\connect pzcrcon
GRANT ALL ON SCHEMA public TO pzcrcon_user;
```

Repository ships `scripts/bootstrap-db.sh` that wraps this and is idempotent (skips if user/db exist). After bootstrap, Prisma migrations run via container entrypoint on each deploy.
...
- [ ] `lib/env.ts` Zod env validator (gates everything else; first task)
- [ ] Next.js scaffold + Tailwind + shadcn/ui themed for design
- [ ] `lib/rcon/` — RCON client wrapper + `players` output parser (own unit, own tests)
- [ ] `lib/auth/` — Auth.js + Discord OAuth + guild + role check + `BOOTSTRAP_OWNER_DISCORD_ID`
- [ ] Prisma schema + migrations + seed + DB bootstrap script
- [ ] Zod schemas for `Player.perks`, `User.themePrefs`, sandbox vars (in `lib/pz/`)
- [ ] Docker dev environment (compose: app + Postgres for local)
...
| `network_mode: host` for RCON | PZ binds RCON only on the public IP `85.215.222.81:27015`, not on `127.0.0.1` | **Decided:** pz-crcon stays on `proxy-net` + `db-net` (bridge), connects to RCON via the public IP. NPM proxies in via container DNS. Verified working (existing `itzg/rcon` uses identical approach). |
| Owner bootstrap race | First-login-wins could let an unauthorized Discord member become OWNER if they hit the URL before the real owner | `BOOTSTRAP_OWNER_DISCORD_ID` env var: only that Discord ID can be assigned OWNER on first login; all other first-time logins become VIEWER pending OWNER promotion |
| Lua mod HMAC secret on disk | `Zomboid/Server/PZCrcon.cfg` stores the webhook secret in plaintext on the PZ server | Accepted: server is single-tenant, root-owned filesystem. Documented in §12. |
...
3. Owner can execute any RCON command via the web terminal and see streamed output **within 500 ms p95** (measurable via E2E)
...
## 16. Required environment variables (mechanically derives `.env.example`)

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `NODE_ENV` | yes | `production` | `development` / `production` |
| `APP_URL` | yes | — | Public base URL, e.g. `https://pz.majorluk.pl` |
| `NEXTAUTH_SECRET` | yes | — | 32-byte random; sign session JWTs |
| `DATABASE_URL` | yes | — | `postgresql://pzcrcon_user:...@shared-postgres:5432/pzcrcon` |
| `DISCORD_CLIENT_ID` | yes | — | Discord OAuth app client ID |
| `DISCORD_CLIENT_SECRET` | yes | — | Discord OAuth app secret |
| `DISCORD_GUILD_ID` | yes | — | Required guild ID; users must be members |
| `DISCORD_ADMIN_ROLE_ID` | yes | — | Discord role granting admin access |
| `DISCORD_BOT_TOKEN` | yes | — | Bot token (for guild membership + role checks) |
| `BOOTSTRAP_OWNER_DISCORD_ID` | yes (first deploy) | — | Discord user ID who becomes OWNER on first login |
| `RCON_HOST` | yes | `85.215.222.81` | PZ RCON address |
| `RCON_PORT` | yes | `27015` | PZ RCON port |
| `RCON_PASSWORD` | yes | — | PZ RCON password |
| `WEBHOOK_HMAC_SECRET` | yes | — | 32-byte random; verifies Lua mod webhook |
| `BACKUP_PATH` | no | `/var/lib/pz-crcon/backups` | Where to store backups |
| `BACKUP_RETENTION_DAYS` | no | `14` | Auto-delete backups older than this |
| `LOG_LEVEL` | no | `info` | Pino log level |
| `WS_HEARTBEAT_SEC` | no | `30` | WS keepalive interval |

`.env.example` ships pre-populated with the non-secret defaults and `CHANGE_ME` for secrets.
...
2. ✅ Spec review by reviewer subagent
