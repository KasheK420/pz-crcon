...
// Server-only RCON command helpers (servermsg, saveWorld, quitServer,
// reloadOptions, rconPing) live in `./server-commands`. They are kept out
// of this module so it stays free of Node-only transitive deps (`net` via
// `rcon-client`) and can be safely imported from client components such as
// the terminal and cheat sheet.
